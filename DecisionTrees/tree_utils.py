...
class InternalDecisionNode(object):
    ...
    def predict(self, x_TF):
        ...
        # Determine which examples go to left child and which go to right
        ...
        # Get predictions from left child for left examples
        if np.sum(left_mask_T) > 0:
            yhat_T[left_mask_T] = self.left_child.predict(x_TF[left_mask_T])
        
        # Get predictions from right child for right examples
        if np.sum(right_mask_T) > 0:
...
class LeafNode(object):
    ...
    def predict(self, x_TF):
        ...
        # Use the mean of all y values that reached this leaf during training
