...
def select_best_binary_split(x_NF, y_N, MIN_SAMPLES_LEAF=1):
    ...
    for f in range(F):
        ...
        # For each threshold, split the data and compute squared error cost
        ...
        for v in range(V):
            ...
            # Get y values for left and right splits
            y_left = y_N[left_mask]
            y_right = y_N[right_mask]
            
            # Compute predictions (mean of y values in each split)
            left_yhat_V[v] = np.mean(y_left) if len(y_left) > 0 else 0.0
            right_yhat_V[v] = np.mean(y_right) if len(y_right) > 0 else 0.0
            
            # Compute squared error cost for each split
            left_cost_V[v] = np.sum(np.square(y_left - left_yhat_V[v])) if len(y_left) > 0 else 0.0
            right_cost_V[v] = np.sum(np.square(y_right - right_yhat_V[v])) if len(y_right) > 0 else 0.0
        ...
        # Pick out the split candidate that has best (minimum) cost
